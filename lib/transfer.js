...
function signTransfer(state, multisigs, fromIndex, bundles) {
  let promises = bundles.map((bundle, i) => {
    return new Promise( (resolve, reject) => {
      let multisig = multisigs[i + multisigs.length - bundles.length];
...
function checkTransfer() {
  ...
  // TODO 
  // Check if the modification of the state is consistent with deposits 
  const transferBundle = bundles[bundles.length];
  const remainderTransaction = transferBundle.filter(tx => tx.address == state.remainderAddress)[0];
  ...
  })(state.transfers[state.transfers.length]);
  let comparisonIndex = state.outputAddresses.indexOf(diffFromPreviousTransfer[0]);
