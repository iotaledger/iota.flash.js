 ...
 */
function compose(balance, deposit, outputs, stakes, root, remainder, history, transfers) {
  ...
  const bundles = [];
  let multisigs = getLastBranch(root);
...
function applyTransfers(root, deposit, stakes, outputs, remainder, history, transfers) {
  ...
  if (transfers.filter(transfer => 
      ...
      .filter(tx => !IOTACrypto.utils.validateSignatures(transfer, tx.address)).length != 0)
  ) {
...
function getDiff(root, remainder, history, bundles) {
  ...
  const newCopy = helpers.deepClone(lastTransfer.filter(tx => tx.value > 0).map(tx => Object({address: tx.address, value: tx.value})));
  ...
  const negatives = newCopy.filter(tx => tx.value < 0 && tx.address != remainder.address);
