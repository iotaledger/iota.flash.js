...
function getTransferDiff(state, multisigs, bundles) {
  ...
  let previousTransfer = state.transfers[state.transfers.length-1];
  let lastTransfer = bundles[bundles.length-1]
  let previousRemainder = previousTransfer.filter(tx => tx.address == state.remainderAddress && tx.value > 0)[0]
  let newRemainder = lastTransfer.filter(tx => tx.address == state.remainderAddress && tx.value > 0)[0]
  if(newRemainder.value > previousRemainder.value) {
    return Promise.reject("Remainder should not increase");
  }
  let newCopy = lastTransfer.deepClone();
  previousTransfer.filter(tx => tx.value > 0).map(tx => {
    var existing = newCopy.filter(t => t.address == tx.address);
  ...
  let negatives = newCopy.filter(tx => tx.value < 0);
  if(negatives.length > 0 ) {
    return Promise.reject("Output diffs must be greater than zero except for remainder");
  }
  return newCopy;
}
