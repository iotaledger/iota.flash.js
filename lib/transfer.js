...
function compose(balance, deposit, outputs, root, remainder, history, transfers, close) {
  ...
  let outputCopy = helpers.deepClone(outputs);
  ...
  transfers = transfers.map( transfer => {
    if (!(transfer.address in outputCopy)) {
      outputCopy[transfer.address] = 0;
    }
    const addedValue = transfer.value;
    transfer.value += outputCopy[transfer.address];
    outputCopy[transfer.address] += addedValue;
    return transfer;
  });
  const bundles = [];
 ...
 */
function sign(root, seed, bundles, userIndex) {
  ...
  return helpers.deepClone(bundles).map((bundle, i) => {
    ...
    let signingIndex = userIndex * multisig.security    
    let signedTx = bundle.filter(tx => tx.address == multisig.address);
    for(let j = 0; j < signingIndex; j++) {
      if(
        IOTACrypto
        .utils
        .inputValidator
        .isNinesTrytes(signedTx[j].signatureMessageFragment)
      ) {
        signedTx[j].signatureMessageFragment = 
          signedTx[j].signatureMessageFragment.replace(/^9/,'A');
      }
    }
    return { bundle: bundle[0].bundle,
      address: multisig.address,
      index: signingIndex,
      signatureFragments: IOTACrypto.multisig.addSignature(
        bundle, 
        multisig.address, 
      ...
      .slice(signingIndex, signingIndex + multisig.security)
    }
  });
}
  ...
  return helpers.deepClone(bundles).map((bundle, i) => {
    ...
    if (userSignature) {
      ...
      bundle
        ...
        .slice(userSignature.index, userSignature.signatureFragments.length)
        .map((tx,j) => tx.signatureMessageFragment = userSignature.index + userSignature.signatureFragments.length);
      // add signature
    }
    return bundle;
  });
