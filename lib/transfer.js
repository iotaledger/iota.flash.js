...
function prepare(settlement, stakes, deposits, fromIndex, destinations) {
  ...
  const total = transfer.reduce((acc, tx) => acc + tx.value, 0);
  ...
  // copy destinations
  const transfer = deepClone(destinations);
  // add deposit release to outputs
  settlement.map((s, i) => {
    const current = transfer.find(tx => tx.address == s.address);
    const stake = stakes[i] * total;
    if(current) {
      current.value += stake;
    } else {
      transfer.push({ address: current.address, value: stake})
    }
  });
