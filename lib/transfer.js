 ...
 */
function compose(state, multisigs, fromIndex, transfers) {
 ...
 * @param {array} transfers
 */
function apply(state, transfers) {
...
function getMultisigs(root, transfers) {
  ...
  let node;
  for(node = root; node.address != transfers[0].address && node.children.length != 0; node = node.children[node.children.length - 1]) {}
...
function sign(seed, index, security, multisigs, fromIndex, bundles) {
