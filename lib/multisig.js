...
function initializeAddress(flash, digests) {
  const address = new IOTACrypto.multisig.address();
  const multisig = {
    'address': address,
    'securitySum': flash.state.security,
    'index': flash.state.index++,
    'children': [],
    'bundles': [],
  }
  if (digests) {
...
function initializeTree(flash, depth) {
  const root = initializeAddress(flash);
  let node = root;
  for(let i = depth; i-- > 0;) {
    node.children.push(initializeAddress(flash));
...
function updateLeafToRoot(flash) {
  ...
  for(i = multisigs.length; i-- > 0 && multisigs[i].bundles.length == 3;) {
    multisigs[i] = initializeAddress(flash);
    if(i > 0) {
